...
**This also fixes a live bug in sun-shine.** `createRouter` reads
`authRepository.isSignedIn` in its `redirect`, but passes no
`refreshListenable`, and `main.dart:30` builds the router once in
`initState`. A successful sign-in therefore never re-triggers the redirect.
`AuthScope`'s `ValueKey(userId)` rebuild cannot help — the router sits above
it.
